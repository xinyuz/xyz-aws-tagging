...
public class ObjectTaggingIntegrationTest extends S3IntegrationTestBase {
    ...
    public void multipartUploadWithNewTags_shouldSucceed() {
        ...
        String key      = makeNewKey();
        String uploadId = s3.createMultipartUpload(b -> b.tagging(tags).bucket(BUCKET).key(key)).uploadId();
        ...
        CompletedMultipartUpload parts              = CompletedMultipartUpload.builder().parts(p -> p.partNumber(1).eTag(uploadPartResponse.eTag()).build()).build();
